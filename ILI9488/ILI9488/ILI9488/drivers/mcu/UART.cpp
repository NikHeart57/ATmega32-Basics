/**
 * @file        UART.cpp
 * @brief       Реализация драйвера UART для ATmega32
 * @version     1.1
 */
              ...
              UART_Interrupt interrupt)
{
    ...
    UCSRC = (1 << URSEL) | (parity << UPM0) | 
            (stopBits << USBS) | (dataBits << UCSZ0);
...
void UART_Init(UART_State state)
{
    if(state == UART_ENABLED) {
        // Расчет делителя для 9600 бод при текущей F_CPU
 ...
 * @note Блокирует выполнение до появления данных
 */
